<script lang="ts">
  ...
  let resolved_routes = $derived(routes_with_title());
</script>
...
<div
  class={cn(
    "bg-background fixed top-0 left-0 z-50 flex min-h-dvh min-w-[80vw] flex-col",
    "border-r pt-[calc(var(--spacing)*15-0.5px)] transition-transform duration-300",
    "ease-in-out sm:min-w-[60vw] md:min-w-[40vw] lg:sticky lg:top-15 lg:z-auto",
    "lg:h-[calc(100dvh-var(--spacing)*15)] lg:min-h-0 lg:min-w-0 lg:translate-x-0",
    "lg:pt-0",
    class_name,
  )}
  ...
  <div
    class={cn(
      "bg-background hover:bg-surface text-primary-foreground fixed top-[45dvh]",
      "left-[80vw] flex h-[10dvh] w-8 cursor-pointer items-center justify-center",
      "rounded-r-xl border-t border-r border-b opacity-0",
      "transition-[background-color,opacity] duration-200 sm:left-[60vw]",
      "md:left-[40vw] lg:hidden",
    )}
    onclick={e => {
      e.stopPropagation();
      SIDEBAR.open = false;
  ...
  {#each resolved_routes as item, index}
    {#if !item.hidden}
      <a
        use:link
        use:active
        class={cn(
          "hover:from-surface [&.active]:from-surface/70 [&.active]:hover:from-surface",
          "border-border block border-b bg-linear-to-l from-transparent to-transparent",
          "py-4 pl-4 focus:ring-0 focus:outline-none",
